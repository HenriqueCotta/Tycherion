...
from tycherion.adapters.mt5.watchlist_mt5 import MT5Watchlist
from tycherion.application.usecases.run_sma_cross import run_usecase_sma_cross
...
def run_app(config_path: str) -> None:
    ...
    try:
        ...
        account = MT5Account()
        watchlist = MT5Watchlist()

        name = cfg.application.usecase.name.lower()
        if name == "sma_cross":
            run_usecase_sma_cross(cfg, market_data, trader, account, watchlist)
        else:
            raise SystemExit(f"Unknown usecase: {name}")
    finally:
        mt5.shutdown()
